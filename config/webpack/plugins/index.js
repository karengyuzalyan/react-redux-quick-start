export { default as progressBarPlugin } from './progres-bar.plugin';
export {
  cleanPlugin
} from './clean.plugin';
export { default as htmlPlugin } from './html.plugin';
export { default as extractCssPlugin } from './extract-css.plugin';
export { default as cssoWebpackPlugin } from './csso-webpack.plugin';
export { default as htmlInlineSrcPlugin } from './html-inline-src.plugin';
export { default as statsWriterPlugin } from './stats-writer.plugin';
export { default as faviconsPlugin } from './favicons.plugin';
export { default as copyPlugin } from './copy.plugin';
export { default as bundleAnalysePlugin } from './bundle-analyse.plugin';
export { getCircularDependenciesPlugin } from './circular-dependencies.plugin';
